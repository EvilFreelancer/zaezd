...
6. **Run the full gate.**
   ...
   the browser build, bdd, unit tests). Everything green, no exceptions. The browser tree is
   compiled before the specifications because one of them asks the server for the compiled
   `boot.js`, and on a clean clone that file does not exist yet.
