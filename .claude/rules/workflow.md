...
6. **Run the full gate.**
   `npm run verify` (typecheck, lint, bdd, unit tests). Everything green, no exceptions.
...
1. Identify every rule tree in the repo: `.cursor/rules/`, `.claude/rules/`, root
   `AGENTS.md` / `CLAUDE.md`, the Codex bridge (`.codex/`), plus any other agent roots
   present (`.github/copilot-instructions.md`, `.opencode/`).
2. For each edited file, locate or create its counterpart in every other tree under the
   same topic name (`workflow`, `bdd-specs`, `testing`, `architecture`, `code-style`,
   `implementation-order`, `composer-core`, `data-sources`, `mcp-layer`, `web-ui`).
3. Copy the body verbatim, then adapt frontmatter and inline links:
   - Cursor `alwaysApply: true` without `globs` <-> Claude rule **without** `paths:`;
   - Cursor `globs: src/**/*.ts` + `alwaysApply: false` <-> Claude `paths: ["src/**/*.ts"]`;
   - Cursor `@file.mdc` references <-> Claude `.claude/rules/file.md` references;
   - file extension `.mdc` <-> `.md`.
4. Keep the language identical across trees. Rule files and `AGENTS.md` are written in
   **English**; chat replies stay in Russian.
5. If `AGENTS.md` changed, verify `CLAUDE.md` still resolves to it
   (`ls -la CLAUDE.md` shows `CLAUDE.md -> AGENTS.md`; restore with `ln -sf AGENTS.md CLAUDE.md`).
6. If a rule file was added, renamed or removed, refresh the index in `.codex/rules.md`.
   The hook itself (`.codex/hooks.json`, `.codex/hooks/attach_rules.py`) reads
   `.cursor/rules/` directly and needs no update.
7. Commit both sides together and list every synced file in the report.

Skip only when a topic is genuinely tool-specific. When skipping, leave a one-line comment
in the file that diverges so the divergence is visible and intentional.
