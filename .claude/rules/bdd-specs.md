...
Every use case below is specified in `features/` and runs offline. The list is the acceptance
surface of the product, derived from `specs/10-proverka.md` and `specs/04-kompozitor.md`.

| Feature file | What it holds |
|---|---|
| `stay-dates.feature` | check-in, check-out, nights, unknown start time |
| `event-selection.feature` | online, origin city, the window, the cap of five |
| `feasibility.feature` | arrival before the opening, departure after the end, unknown times |
| `trip-price.feature` | both legs plus the whole-stay hotel, event price as text, budget |
| `hotel-choice.feature` | ranking by distance, and by price when there is no distance |
| `three-packages.feature` | the three rules and their collapse to fewer cards |
| `checkout-labels.feature`, `checkout-links.feature` | the label a `kind` earns, and the live build |
| `source-payloads.feature`, `source-clients.feature` | normalization, sessions, one call at a time |
| `recorded-sources.feature` | cache keys, equivalent argument shapes, replay |
| `optional-context.feature` | geocoding, walking time, working days, forecast |
| `trip-assembly.feature` | the whole product on recordings, including failing sources |
| `trip-board.feature` | the screen: naming, escaping, stages, the request row |
| `agent-tools.feature` | the three tools, their honesty, the published address |
| `trip-widget.feature` | the `ui://` resource and what travels beside the answer |
| `harness.feature` | the tooling's own self-check |

A behaviour that is not in this table and not in a `.feature` file does not exist. When one is
