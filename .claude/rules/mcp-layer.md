...
**same HTML** as the web screen. Two copies would drift and the UX score would suffer on
...
CSP is the main practical risk: the host defaults to `default-src 'none'`, which breaks
MapLibre first (blob: workers, WebGL, external tiles). Verify in a real MCP host, since a
plain browser does not reproduce the iframe restrictions. Order of work: browser first,
then the same page in an iframe with a deliberately strict CSP, then inside the agent.
If the map cannot start there, the MCP App falls back to a static map image with an "open
in full" link, and that fallback is prepared in advance, not improvised.
